...
@section('page-title', 'Customer Listing')

@section('content')

<div x-data="customerSyncPage">
...
<div class="bg-white rounded-xl border border-gray-200 p-4 mb-4 shadow-sm">
    <form method="GET" class="flex flex-wrap items-end gap-3">
        ...
        <div>
            ...
            <input type="text" name="search" value="{{ $search }}" placeholder="Name, email, ID…" class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-64 focus:ring-2 focus:ring-indigo-300 outline-none">
        </div>
        <div>
        ...
        <a href="{{ route('dashboard.customers') }}" class="bg-gray-400 text-white px-4 py-[7px] rounded-lg text-sm hover:bg-gray-500 transition">Reset</a>
    </form>
</div>
    ...
    <div class="overflow-x-auto min-h-[200px]">
        <table class="w-full text-sm">
            ...
            <tbody id="customers-table-body" class="divide-y divide-gray-100">
                @include('dashboard.partials.customers-table-rows', compact('customers', 'syncMode', 'direction', 'erpDisplayName', 'ecomDisplayName'))
            </tbody>
        </table>
    </div>
...
@php
    $pushUrlTemplateEcom = str_replace('/0/post', '/{id}/post', route('dashboard.customers.post-single', ['id' => '0']));
    $pushUrlTemplateErp  = str_replace('/0/post', '/{id}/post', route('dashboard.customers.post-single', ['id' => '0']));
@endphp

<script>
document.addEventListener('alpine:init', () => {
    Alpine.data('customerSyncPage', () => ({
        ...
        bulkDeleteUrl: @json(route('dashboard.customers.destroy-bulk')),
        deleteLabel: 'customer',
        openToolId: null,
        toast: { show: false, level: 'success', message: '' },
        toastTimer: null,
        tableRowsUrl: @json(route('dashboard.customers.rows')),
        ...
        actions: {
            pull: {
                url: @json(route('dashboard.customers.pull')),
                body: null,
            },
            fetch: {
                url: @json(route('dashboard.customers.fetch')),
                body: null,
            },
            postErp: {
                url: @json(route('dashboard.customers.post')),
